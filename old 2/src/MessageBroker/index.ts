...
// @ts-ignore
import { Context } from "../../Domain/interfaces";
...
export default class MessageBroker implements IMessageBroker {
  ...
  constructor(queueName: string, client: amqp.Connection) {
    if (!queueName) {
  ...
  use(route: string | null, handle: AMQPHandle): void {
    if (!route) {
      route = "/";
    }
