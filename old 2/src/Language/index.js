...
import { LanguageMap, type ISOCodes } from "./data";
import Normalize from "../Normalize";

const LanguageCodes = Object.keys(LanguageMap);
const LanguageNames = Object.values(LanguageMap);
...
LanguageCodes.forEach(key => {
  ...
  names.forEach(name => (ReverseLanguageMap[name] = key));
});
...
export default class Language
  ...
  constructor(
    ...
    if (from === "serialized") {
      ...
      this._id = ReverseLanguageMap[this.name];
  ...
  get name(): string {
    return LanguageMap[this.id].name;
  }
