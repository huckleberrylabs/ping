...
const extractAllNumbers = (string: { match: (arg0: RegExp) => void }) =>
  string.match(/\d+/g);

const saveProfile = async (browser: any, href: any) => {
  // debug('Hey', href);
...
const crawlProfiles = async (browser: any, numPages: any) => {
...
const scrape = (indexArray: any) => {
  async.eachLimit(
    indexArray,
    2,
    async (index: any, done: any) => {
    ...
    (err: any) => (err ? debug(err) : debug("done"))
  );
};

const recursiveScrape = async (oldSearch: string) => {
  async.each(CHARS.split(""), async (char: any) => {
...
const recursiveIndex = (oldSearch: string, browser: { newPage: () => any }) => {
...
const flattenIndex = (index: any) => {
  if (typeof index.records === "number" && index.records > 0) {
    return index;
  } else if (Array.isArray(index.records)) {
    const result = index.records.map((item: any) => flattenIndex(item));
    return [].concat(...result);
  } else if (Array.isArray(index)) {
    const result: any = index.map(item => flattenIndex(item));
    return [].concat(...result);
  }
};
...
const searches = flattenIndex(index).filter((i: any) => i !== undefined);
debug(
  searches
    .map((i: { records: any }) => i.records)
    .reduce((a: any, b: any) => a + b, 0)
);
const lastSearchIndex = searches.findIndex(
  (x: { search: string }) => x.search === "zi"
);
