...
const scrapeLawyerProfile = async (request, callback) => {
  ...
  // Name
  const name = tableRowsText.filter(text => text.indexOf("Name:") > -1);
  if (name.length > 0) {
  ...
  const calledToBar = tableRowsText.filter(
    text => text.indexOf("Called to Bar:") > -1
  );
  ...
  // Degrees
  const degrees = tableRowsText.filter(text => text.indexOf("Degrees:") > -1);
  if (degrees.length > 0) {
    response.degrees = degrees[0].replace("Degrees:", "").trim();
  }

  // Phone
  const phone = tableRowsText.filter(text => text.indexOf("Tel:") > -1);
  if (phone.length > 0) {
    response.phone = phone[0].replace("Tel:", "").trim();
  }

  // Email
  const email = tableRowsText.filter(text => text.indexOf("Email:") > -1);
  if (email.length > 0) {
    response.email = email[0].replace("Email:", "").trim();
  }

  // Firm
  const firm = tableRowsText.filter(text => text.indexOf("Company:") > -1);
  if (firm.length > 0) {
  ...
  // Website
  const website = tableRowsText.filter(text => text.indexOf("Website:") > -1);
  if (website.length > 0) {
  ...
  const address = tableRowsText
    .filter(text => text.indexOf("Address") > -1)
    .map(text =>
      text
        .replace("Address 1:", "")
        .replace("Address 2:", "")
  ...
  // City
  const city = tableRowsText.filter(text => text.indexOf("City:") > -1);
  if (city.length > 0) {
    response.city = city[0].replace("City:", "").trim();
  }

  // Province
  const province = tableRowsText.filter(text => text.indexOf("Province:") > -1);
  ...
  const postalCode = tableRowsText.filter(
    text => text.indexOf("Postal Code:") > -1
  );
  ...
  const specialties = tableRowsText
    .filter(text => text.indexOf("area of practice:") > -1)
    .map(text =>
      text
  ...
  if (
    specialties.filter(text => text.indexOf("Not suppplied by lawyer") > -1)
      .length === 0
  ) {
    response.specialties = [...new Set(specialties)];
  }

  // Free Consultation
  const consultation = tableRowsText
    .filter(
      text => text.indexOf("Provides a free initial consultation?") > -1
    )[0]
  ...
  const legalAid = tableRowsText
    .filter(text => text.indexOf("Accept Legal Aid clients?") > -1)[0]
...
// Returns the Profile URIs
const generateProfileURIs = async (request, callback) => {
...
const transformer = async (request, callback) => {
  if (requestIsValid(request)) {
    ...
    if (typeof request.specialties !== "undefined") {
      personTags = request.specialties.filter(
        text => text.length >= 1 && text.split(" ").length <= 3
      );
      const extractedKeywords = await Promise.all(
        request.specialties
          .filter(text => text.split(" ").length > 3)
          .map(text => keywordExtractor(text))
      );
      personTags = [].concat.apply(
        personTags,
        extractedKeywords.map(e => e.keyphrases)
      );
      personTags = personTags.map(text => norm(text));
      personTags = [...new Set(personTags)];
    }
