import Task, { type TaskArrayPromise } from "../../../Entities/Task";
...
export async function validateURL(url: string): Promise<boolean> {
  try {
    ...
    const isValid =
      ...
      SEARCH_PARAM_TYPES.includes(input.searchParams.get("searchtype")) &&
      SEARCH_PARAM_PROVINCES.includes(input.searchParams.get("province"));
    return isValid;
  } catch (error) {
    return false;
  }
...
export async function process(params: { url: string }): TaskArrayPromise {
  ...
  const allLinks = $("a")
    ...
    .map(href => ["https://canadianlawlist.com", href].join(""));
