...
export const NetworkInformationCodec = iots.type({
  downlink: iots.number,
  effectiveType: iots.string,
  rtt: iots.number,
  saveData: iots.boolean,
});
export type NetworkInformation = iots.TypeOf<typeof NetworkInformationCodec>;

export const NetworkCodec = iots.type({
  connection: optionFromNullable(NetworkInformationCodec),
  localIP: optionFromNullable(iots.string),
});

// https://github.com/diafygi/webrtc-ips
export const LocalIP = async (): Promise<Option<string>> => {
