...
export const Name = "error:not-implemented" as NameSpaceCaseString.T;
...
export const Decode = (value: unknown) =>
  fold<iots.Errors, T, Either<Validation.T, T>>(
    errors =>
      left(Validation.C(Name, `Decode: ${DecodeErrorFormatter(errors)}`)),
    decoded => {
      Logger(
        decoded.origin,
        "error",
...
export const C = (
  ...
  return {
    ...
    userMessage: userMessage
      ? userMessage
      : "Software feature not implemented yet, Please check back at a later date.",
    type: Name,
  };
};
